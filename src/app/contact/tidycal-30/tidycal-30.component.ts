...
export class Tidycal30Component implements AfterViewInit, OnDestroy {
  @Input() src = 'https://tidycal.com';
  @Input() initialHeight = 1080;
  @Input() expandedHeightMobile = 1800;
  ...
  // Wichtig für das HTML @if Schutzschild
  ...
  ngAfterViewInit(): void {
    // Absoluter Stopp für den Server. Nichts, was danach kommt, blockiert Node.js!
    if (!this.isBrowser) {
      return;
    }
    ...
    // Timer und EventListener außerhalb von Angular ausführen,
    // damit Zone.js den SSR-Prozess niemals blockieren kann
    this.ngZone.runOutsideAngular(() => {
      this.resizeListenerUnsub = this.renderer.listen(
        'window',
        'resize',
  ...
  private onIframeUserInteraction() {
    ...
    // setTimeout MUSS außerhalb von Angular laufen, sonst wartet SSR ewig auf das Ende des Timers!
    this.ngZone.runOutsideAngular(() => {
      setTimeout(() => {
        if (this.isMobile() && !this.expanded) {
          this.expandAndScroll('interaction');
  ...
  private setIframeHeight(px: number) {
    if (!this.tidycalFrame || !this.tidycalFrame.nativeElement) return;
    this.tidycalFrame.nativeElement.style.height = px + 'px';
  }

  private scrollToIframeTop(): void {
    if (!this.isBrowser || !this.tidycalFrame) return;
